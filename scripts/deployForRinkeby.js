...
const setupData = async () => {
  const { deployer, users, dai, link, lendingPool } = testEnv;
  // deployer != users[0]
  ...
  console.log("Deployer deposits 10 DAI");
  ...
  const linkTx = await lendingPool
    .connect(users[0].signer)
    .deposit(link.address, toWei(1), users[0].address);
  await linkTx.wait();

  console.log("User 0 deposits 1 LINK");
  ...
  console.log("Deployer borrows 0.2 LINK against DAI as collateral");
};
