...
contract LendingPool is ILendingPool, LendingPoolStorage {
  ...
  function borrow(
    ...
    uint256 amountInETH =
      IPriceOracle(oracle).getAssetPrice(asset).mul(amount).div(
  ...
  function liquidationCall(
    ...
    if (!receiveAToken) {
      uint256 currentAvailableCollateral =
        ...
        require (currentAvailableCollateral >= vars.maxCollateralToLiquidate, Errors.LP_NOT_ENOUGH_LIQUIDITY_TO_LIQUIDATE);
    }
   ...
   **/
  function getAddressesProvider() external view override returns (IAddressesProvider) {
